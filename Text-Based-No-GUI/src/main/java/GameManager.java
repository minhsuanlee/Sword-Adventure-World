...
public class GameManager {
    ...
    switch (action) {
      case 1:
        if (game.getDevilDefeated()) {
          ...
          System.out.println(
              "           Please come in. You are more than welcome here in the Town!");
          game.setIsGameOver();
          printGameStoryDivider();
          printGameNarrative("END OF GAME");
        } else {
          System.out.println(
              "The Guard: Wow wow wow... slow down there \"" + game.getPlayerName() + "\".");
          System.out.println(
              "           We don't allow stranger just walk into the Town like that.");
        }
        break;
      case 2:
        if (game.getGoldCount() > 0) {
          int remainingGoldCount = game.buyInformation();
          ...
          printGameNarrative("You now have " + remainingGoldCount + " gold left");
        } else {
  ...
  public static void sneakIntoDevilsCastle() {
    ...
    if (game.getDevilDefeated()) {
      ...
      for (int i = 0; i < demons.length; i++) {
        ...
        printGameNarrative(
            "Demon " + (i + 1) + " appears. What would you like to do? (Enter Action #)");
        System.out.println();
      ...
      if (game.getHP() > 0 && choice == 1) {
        if (!game.getWeapon().equals("Excalibur")) {
          ...
          System.out.println(
              "The Goddess: Thank you, " + game.getPlayerName() + ", for travelling this far.");
          System.out.println("              I will now restore all your HP and good luck.");
          System.out.println();

          game.restoreFullHP();
          printGameNarrative("The Goddess healed you");
          printGameNarrative("HP: " + game.restoreFullHP());
          printGameStoryDivider();
  ...
  public static void fight(Monster creature) {
    ...
    if (action.compareTo(mAction) > 0) {
      ...
      if (mHP > 0) printGameNarrative("The " + creature + " has " + mHP + " HP left");
